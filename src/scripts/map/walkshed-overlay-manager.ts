...
export class WalkshedOverlayManager {
  ...
  removeStop(stopId: string): void {
    ...
    for (const key of this.unavailableKeys) {
      if (key.startsWith(`${stopId}:`)) this.unavailableKeys.delete(key);
    }
  }
  ...
  private scheduleSyncIfEnabled(): void {
    if (this.isEnabled()) this.scheduleVisibleSync();
  }
  ...
  private enqueueStop(stop: Stop): void {
    ...
    if (this.unavailableKeys.has(`${stop.id}:${this.getRadiusMeters()}`)) return;
    this.pendingStopIds.add(stop.id);
  }

  private dequeueStopId(): string | null {
    const first = this.pendingStopIds.values().next();
    if (first.done) return null;
    this.pendingStopIds.delete(first.value);
    return first.value;
  }
  ...
  private syncVisibleCoverage(): void {
    ...
    const bounds = this.map.getBounds().pad(BOUNDS_PADDING);
    ...
    for (const stopId of this.layersByStopId.keys()) {
      if (!visibleStopIds.has(stopId)) this.removeLayer(stopId);
    }

    for (const stopId of this.pendingStopIds) {
      if (!visibleStopIds.has(stopId)) this.pendingStopIds.delete(stopId);
    }
  ...
  private async loadWalkshedForStop(
    ...
    const stopInBounds =
      ...
      this.map.getBounds().pad(BOUNDS_PADDING).contains([stop.lat, stop.lon]);
    if (!stopInBounds) return;

    if (!polygon) {
      this.unavailableKeys.add(`${stop.id}:${radiusMeters}`);
      return;
    }

    this.unavailableKeys.delete(`${stop.id}:${radiusMeters}`);
  ...
  private drainQueue(): void {
    ...
    while (this.activeWorkers < LOAD_CONCURRENCY) {
      ...
      if (!stop || this.layersByStopId.has(stopId) || this.inFlightStopIds.has(stopId)) continue;
      if (this.unavailableKeys.has(`${stopId}:${this.getRadiusMeters()}`)) continue;
      ...
      void this.loadWalkshedForStop(stop, version, radius)
        .catch((err) => console.error(`Walkshed failed for ${stopId}:`, err))
        .finally(() => {
